...
export default function GetStartedCode() {
  ...
  return (
    <div className="flex flex-col md:inline-flex md:flex-row rounded-xl md:rounded-full shadow-xl md:whitespace-nowrap text-center bg-gray-900 p-4 md:pr-2 md:pl-6 md:py-2 text-white text-sm lg:text-base mt-6 font-mono gap-4 items-center selection:bg-blue selection:text-white">
      <span>
        <span className="text-blue font-bold">npm</span>{" "}
        <span className="text-brand font-bold">create</span> sanity@latest --
        --template sanity-io/sanity-template-nextjs-clean
      </span>
      <button
        className="text-black flex items-center gap-2 relative cursor-pointer bg-white md:aspect-square px-4 py-2 md:p-2 rounded-xl md:rounded-full hover:bg-yellow transition-colors duration-300"
        onClick={handleCopy}
